...
function xmldb_theme_elearning_upgrade($oldversion) {
    ...
    if ($oldversion < 2013041200) {
        // Migrate logo URL.
        $logo = get_config('theme_elearning', 'logo');
        if ($logo === '') {
            ...
            if ($content = download_file_content($logo)) {
                ...
                $fs = get_file_storage();
                $record = array(
                    'contextid' => context_system::instance()->id, 'component' => 'theme_elearning',
                    'filearea' => 'logo', 'itemid'=>0, 'filepath'=>'/', 'filename'=>$filename);
                $fs->create_file_from_string($record, $content);
                set_config('logo', '/'.$filename, 'theme_elearning');
                unset($content);

            } else {
                unset_config('theme_elearning', 'logo');
            }
        } else {
        ...
        upgrade_plugin_savepoint(true, 2013041200, 'theme', 'elearning');
    }
